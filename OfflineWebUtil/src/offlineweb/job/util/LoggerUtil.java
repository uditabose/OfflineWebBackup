...
public class LoggerUtil {
    ...
    public static LoggerUtil getLoggerUtil(Class theClass) {
        return new LoggerUtil(theClass, null);
    }
     ...
     */
    public synchronized void  logStatus (Job.JobStatus status, Object... statusMessage) {
        if (statusMessage != null && statusLoggerPath != null) {
            BufferedWriter statusLogWriter = null;
            try {
                statusLogWriter = new BufferedWriter(new FileWriter(statusLoggerPath, true));
                statusLogWriter.append("status=" + status.toString());
                statusLogWriter.newLine();
                statusLogWriter.append("time=" + new Date());
                statusLogWriter.newLine();
                statusLogWriter.append("message=" + Arrays.deepToString(statusMessage));
                statusLogWriter.newLine();
